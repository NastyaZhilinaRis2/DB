...
--
-- Хост: 127.0.0.1:3306
-- Время создания: Ноя 12 2024 г., 22:26
...
--
-- Структура таблицы `logs_students`
--
...
--
-- Дамп данных таблицы `logs_students`
--

INSERT INTO `logs_students` (`Id_logs_students`, `Id_students`, `Action`, `TimeAction`) VALUES
...
--
DELIMITER $$
CREATE TRIGGER `Delete_students` AFTER DELETE ON `table_students_zhilina` FOR EACH ROW INSERT INTO `logs_students`(`Id_students`, `Action`) VALUES (OLD.Id_students, CONCAT('Удален студент: ', OLD.Surname, ' ', OLD.Name, ' ', OLD.Patronymic, ' Группа: ', OLD.Group))
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `Insert_students` AFTER INSERT ON `table_students_zhilina` FOR EACH ROW INSERT INTO `logs_students`(`Id_students`, `Action`) VALUES (NEW.Id_students, CONCAT('Добавлен студент: ', NEW.Name, ' ', NEW.Surname, ' ', NEW.Patronymic, ' Группа: ', NEW.Group))
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `Update_students` AFTER UPDATE ON `table_students_zhilina` FOR EACH ROW INSERT INTO `logs_students`(`Id_students`, `Action`) 
...
--
-- Индексы таблицы `logs_students`
--
ALTER TABLE `logs_students`
...
--
-- AUTO_INCREMENT для таблицы `logs_students`
--
ALTER TABLE `logs_students`
